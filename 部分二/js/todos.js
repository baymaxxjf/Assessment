(function(){
	...
	function main(){
		...
		close[close.length-1].addEventListener('click',function(event){//控制事件的删除
		...
		//控制内容的展示
		var toggle=document.getElementsByClassName('toggle');
		toggle[toggle.length-1].addEventListener('change',function(){
			showLefts();//统计未完成事件数
		},true);		
		var view=document.getElementsByClassName('view');
		view[view.length-1].addEventListener('dblclick',function(event){//双击显示内容编辑
			var e=event||window.event;
			var elem=e.srcElement||e.target;
	...
	window.addEventListener('load',function(){ //重新加载时获取数据恢复原来的事件 
		...
		if(data.length!=0){ //如果有值恢复menu栏和全选开关
			...
			for (var i=0;i<close.length;i++) {	//for循环绑定事件		
				close[i].addEventListener('click',function(event){//控制事件的删除
					var e=event||window.event;//防止初始化失败
					var elem=e.srcElement||e.target;//点击或触摸点的元素的节点
					elem=elem.parentNode;							
					var parent=elem.parentNode;
			...
			var view=document.getElementsByClassName('view');
			view[i].addEventListener('dblclick',function(event){//双击显示内容编辑
				var e=event||window.event;
				var elem=e.srcElement||e.target;
				var edit=elem.parentNode.lastChild;
				var toggle=elem.previousSibling;
